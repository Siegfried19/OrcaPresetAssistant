...
- [ ] 真实打印提交、用户预设新增/修改热刷新、3MF 三策略、DPI/主题和升级保留工作区仍需人工验收。
...
- [ ] 刷新显示时间、变更数或失败原因。
- [ ] 仅当前项目不会写永久预设。
- [ ] 更新永久用户预设时先写修改日志，再直接更新工作区 JSON 与匹配 `.info`。
- [ ] 新预设以现有用户预设为来源，身份字段一致且不会复制云同步身份。
- [ ] Orca 关闭时也能完成磁盘修改；打开时面板刷新会定向加载新增和修改预设。
- [x] 只有原生回读与日志一致时，后台记录才进入“Orca 已加载”；切片面板不额外显示文件修改卡片。
- [ ] 当前选中预设有未保存 Orca 编辑时，热刷新拒绝覆盖并显示冲突。
...
- [ ] 刷新不扩大权限。
- [ ] 永久用户预设写入不要求读取当前 Orca 设置，也不要求第二次面板确认。
- [ ] 当前设置只在用户明确要求时读取；当前项目写入仍经过面板确认。
